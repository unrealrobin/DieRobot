...
// Sets default values
ADieRobotCharacterBase::ADieRobotCharacterBase()
{
	PrimaryActorTick.bCanEverTick = false;
}

// Called when the game starts or when spawned
void ADieRobotCharacterBase::BeginPlay()
{
	Super::BeginPlay();
	
}

void ADieRobotCharacterBase::AddOverlayMaterialToCharacter(UMaterialInterface* MaterialToOverlay, float TimeBeforeRemoval)
{
	...
	if (TimeBeforeRemoval > 0)
	{
		...
		if (MaterialToOverlay != nullptr)
		{
			...
			if (CharacterMesh)
			{
				...
				&ADieRobotCharacterBase::RemoveOverlayMaterialFromCharacter, TimeBeforeRemoval);
			}
		}
	}
	else
...
void ADieRobotCharacterBase::RemoveOverlayMaterialFromCharacter()
{
	USkeletalMeshComponent* CharacterMesh = GetMesh();
	if (CharacterMesh)
	{
