...
ADieRobotEnemyCharacter::ADieRobotEnemyCharacter()
{
...
void ADieRobotEnemyCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void ADieRobotEnemyCharacter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

void ADieRobotEnemyCharacter::SelfDestruct()
{
...
void ADieRobotEnemyCharacter::SpawnDamageUI(FDamagePayload DamagePayload)
{
...
void ADieRobotEnemyCharacter::ClearFloatingDamageRef()
{
	FloatingDamageContainerActor = nullptr;
}

void ADieRobotEnemyCharacter::SpawnEffectNameUI(FName EffectName, UStatusEffectBase* StatusEffect)
{
...
void ADieRobotEnemyCharacter::Tick(float DeltaSeconds)
{
	Super::Tick(DeltaSeconds);
}

void ADieRobotEnemyCharacter::SetupCharacterMovementData()
{
...
void ADieRobotEnemyCharacter::SetupCharacterMovementDelegates()
{
	CharacterMovementComponent = GetCharacterMovement();
	if (IsValid(CharacterMovementComponent))
	{
		MovementModeChangedDelegate.AddDynamic(this, &ADieRobotEnemyCharacter::HandleOnMovementModeChanged);
		LandedDelegate.AddDynamic(this, &ADieRobotEnemyCharacter::HandleOnLanded);
	}
	else
	{
...
void ADieRobotEnemyCharacter::HandleOnMovementModeChanged(class ACharacter* Character, EMovementMode PrevMovementMode,
	uint8 PreviousCustomMode)
{
	if (Character)
	{
		ADieRobotAiControllerBase* AiController = Cast<ADieRobotAiControllerBase>(GetController());
...
void ADieRobotEnemyCharacter::HandleOnLanded(const FHitResult& Hit)
{
...
FMissionEventPayload ADieRobotEnemyCharacter:: GenerateDamageEventPayload(FDamagePayload DamagePayload)
{
...
FMissionEventPayload ADieRobotEnemyCharacter::GenerateDestroyEventPayload(FDamagePayload DamagePayload)
{
	FMissionEventPayload Payload;
...
void ADieRobotEnemyCharacter::PopulateCombatEventContextTags(FGameplayTagContainer& ContextTagsContainer, const FDamagePayload& DamagePayload) const
{
	//Hit Enemy Context Tag
	if (this->IsA(ADieRobotEnemyMeleeWeaponBase::StaticClass()))
	{
		ContextTagsContainer.AddTag(FGameplayTag::RequestGameplayTag("Event.Enemy.Carbonite.Bruiser"));
	}
	else if (this->IsA(ADieRobotEnemyRangedBase::StaticClass()))
	{
		ContextTagsContainer.AddTag(FGameplayTag::RequestGameplayTag("Event.Enemy.Carbonite.Shooter"));
	}
	else if (this->IsA(ADieRobotEnemyMeleeBase::StaticClass()))
	{
	...
	if (Src && Src->IsA(ADieRobotPlayableCharacter::StaticClass()))
	{
		ContextTagsContainer.AddTag(FGameplayTag::RequestGameplayTag("Event.Combat.Player"));
		TWeakObjectPtr<ADieRobotPlayableCharacter> Kip = Cast<ADieRobotPlayableCharacter>(Src);
		if (Kip.IsValid())
		{
			//Weapon Context
			TWeakObjectPtr<ADieRobotWeaponBase> Weapon =  Kip->CombatComponent->GetCurrentlyEquippedWeapon();
			if (Weapon.IsValid())
			{
				if (Weapon->IsA(ADieRobotWeaponMeleeBase::StaticClass()))
				{
					ContextTagsContainer.AddTag(FGameplayTag::RequestGameplayTag("Event.Weapon.Melee.Hammer"));
				}
				if (Weapon->IsA(ADieRobotWeaponRangedBase::StaticClass()))
				{
...
void ADieRobotEnemyCharacter::TakeDamage(FDamagePayload DamagePayload)
{
...
bool ADieRobotEnemyCharacter::HandleAggroCheck(AActor* DamageInstigator, float DamageReceived, float fDamageAccumulatedDuringWindow)
{
	//If DamageInstigator is a player character, check if the enemy should aggro the player.
	if (Cast<ADieRobotPlayableCharacter>(DamageInstigator))
	{
...
void ADieRobotEnemyCharacter::ResetDamageWindow()
{
	DamageAccumulatedDuringWindow = 0.f;
}

void ADieRobotEnemyCharacter::StartDamageTimerWindow()
{
	...
	if(!bIsTimerActive)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(DamageWindowTimerHandle, this, &ADieRobotEnemyCharacter::ResetDamageWindow, DamageWindowTime, false);
	}
	else
	{
...
void ADieRobotEnemyCharacter::PlayProjectileHitSound(FHitResult HitResult)
{
...
void ADieRobotEnemyCharacter::PlayMeleeWeaponHitSound(FHitResult HitResult)
{
...
void ADieRobotEnemyCharacter::OnDeath_HandleCollision()
{
...
void ADieRobotEnemyCharacter::OnDeath_DropLoot()
{
...
void ADieRobotEnemyCharacter::ScaleHealth()
{
...
void ADieRobotEnemyCharacter::HandleRemoveStatusEffectComponent()
{
...
void ADieRobotEnemyCharacter::HandleToggleDataView(FInputActionValue Input)
{
...
void ADieRobotEnemyCharacter::HandleEnemyDeath()
{
	HandleWeaponDestruction();
	Destroy();
}

void ADieRobotEnemyCharacter::HandleWeaponDestruction()
{
	ADieRobotEnemyMeleeWeaponBase* MeleeWeaponEnemy = Cast<ADieRobotEnemyMeleeWeaponBase>(this);
	ADieRobotEnemyRangedBase* RangedWeaponEnemy = Cast<ADieRobotEnemyRangedBase>(this);
...
float ADieRobotEnemyCharacter::CalculateOutputDamage(float Damage)
{
	return Damage;
}

void ADieRobotEnemyCharacter::StopAiControllerBehaviorTree()
{
	ADieRobotAiControllerBase* AiController = Cast<ADieRobotAiControllerBase>(GetController());
...
void ADieRobotEnemyCharacter::PlayMontageAtRandomSection(UAnimMontage* Montage)
{
	if(IsValid(Montage))
	{
...
ADieRobotBuildingComponentBase* ADieRobotEnemyCharacter::LineTraceToSeeda()
{
	FVector RaycastStart = RaycastStartPoint->GetComponentLocation();

	//Get Seeda Location
	FVector RaycastEnd = Cast<ADieRobotGameModeBase>(GetWorld()->GetAuthGameMode())->SeedaLocation;
	...
	if (bHit)
	{
		for (FHitResult HitActors : HitResults)
		{
			...
			if (HitActors.GetActor()->IsA(ADieRobotBuildingComponentBase::StaticClass()))
			{
				return Cast<ADieRobotBuildingComponentBase>(HitActors.GetActor());
			}
		}
	}
...
void ADieRobotEnemyCharacter::SweepForActor(TSubclassOf<AActor> ActorToSweepFor, float SphereRadius, float DistanceToCheck)
{
...
void ADieRobotEnemyCharacter::SetupDataClusterComponent()
{
	if (DataClusterWidgetComponent)
	{
		...
		if (IsValid(PlayerCharacter))
		{
			...
			if (IsValid(PC))		
			{
				PC->ToggleDataView_DelegateHandle.AddDynamic(this, &ADieRobotEnemyCharacter::HandleToggleDataView);
			}
		}
