...
// Sets default values
ADieRobotEnemyRangedBase::ADieRobotEnemyRangedBase()
{
...
void ADieRobotEnemyRangedBase::BeginPlay()
{
	Super::BeginPlay();
	EquipRangedWeapon();
...
void ADieRobotEnemyRangedBase::EquipRangedWeapon()
{
	...
	if(RangedWeaponClassName != nullptr)
	{
		...
		ADieRobotWeaponBase* RangedWeaponInstance = GetWorld()->SpawnActor<ADieRobotWeaponBase>(
		...
		{
			EquippedWeapon = Cast<ADieRobotWeaponRangedBase>(RangedWeaponInstance);
			if (EquippedWeapon)
			{
...
void ADieRobotEnemyRangedBase::GetRotationToCurrentTarget()
{
...
// Called every frame
void ADieRobotEnemyRangedBase::Tick(float DeltaTime)
{
